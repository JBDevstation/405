 ...
 * kr_malloc is aligned properly for the objects that are stored in it, all
...
void kr_free(void *ptr) {
   ...
   * kr_malloc because the next search in the iteration looks at exactly the
   * right memory block.
   */
  freep = currp;
}

void print_kr_malloc() {
  Header *currp = NULL;
  currp = freep;
  for (int i = 0; i < 4; i++) {
    printf("currp: %p, currp+1: %p, currp->size: %d, currp->next: %p\n", currp, currp+1, currp->s.size, currp->s.next);
    currp = currp->s.next;
    if (currp == freep)
      break;
  }
  /*
  for (currp = freep; currp != NULL; currp = currp->s.next) {
...
int main() {
    ...
    int *p1 = kr_malloc(sizeof(int));
    *p1 = 1;
    printf("p1: %p, *p1: %d\n", p1, *p1);

    print_kr_malloc();

    int *p2 = kr_malloc(sizeof(int));
    *p2 = 2;
    printf("p2: %p, *p2: %d\n", p2, *p2);

    int *p3 = kr_malloc(sizeof(int));
    *p3 = 3;
    printf("p3: %p, *p3: %d\n", p3, *p3);

    kr_free(p1);

    int *p4 = kr_malloc(sizeof(int));
    *p4 = 4;
    printf("p4: %p, *p4: %d\n", p4, *p4);

    long *lp1 = kr_malloc(sizeof(long));
    *lp1 = 10;
    printf("lp1: %p, *lp1: %ld\n", lp1, *lp1);

    long *lp2 = kr_malloc(sizeof(long));
    *lp2 = 20;
    printf("lp2: %p, *lp2: %ld\n", lp2, *lp2);

    long *lp3 = kr_malloc(sizeof(long));
    *lp3 = 30;
    printf("lp3: %p, *lp3: %ld\n", lp3, *lp3);

    kr_free(lp1);

    long *lp4 = kr_malloc(sizeof(long));
    *lp4 = 40;
    printf("lp4: %p, *lp4: %ld\n", lp4, *lp4);

    printf("lp4: %p, lp4[-1]: %ld\n", lp4, lp4[-1]);

    int *p = kr_malloc(sizeof(int));
    *p = 100;
    for (int i = 0; i < 45; i++)
        printf("p: %p, p[i]: %d\n", p+i, p[i]);
