...
bool enqueue_proc(struct proc *p) {
    // design and implement this function
    struct proc *current = kernel_proc;
    struct proc *prev = kernel_proc;
    while(current != NULL){
    	//Ready to insert proc into the list
	...
	//Move to next section of the list
	prev = current;
	current = current -> next;
    }
    //Insert p at the last spot in the list
    prev -> next = p;
    p -> prev = prev;
    return true;
}
